<?php 
session_start(); 
include($_SERVER['DOCUMENT_ROOT']."/tgt/classes/db_config.php"); 
class Common { 
	  public $conn;
 ...
 function CheckUserExist($email)
	{
		 $sql = "SELECT email FROM tbl_registration WHERE email='".$email."'";
		  ...
		  $get_email = mysqli_query($this->conn,$sql) or die(mysqli_connect_errno()."Data cannot inserted");
	...
	function UserLogin($username,$password)	 
	   { 
	   $msg="";
		$sql = "SELECT * FROM tbl_registration WHERE email='".$username."'";	  
		$check_user = mysqli_query($this->conn,$sql) or die(mysqli_connect_errno()."Data cannot inserted"); 
		   if(mysqli_num_rows($check_user))
		     {		 	 
		     $sql = "SELECT * FROM tbl_registration WHERE email='".$username."' AND password='".$password."'";
					   ...
					   if(mysqli_num_rows($check_password))
					    {
						  ...
						  $_SESSION['loginuser']=$user_det;
						   //echo "<pre>";print_r($user_det);echo "</pre>"; die(); 
						 }else{
	...
	function AllData($tbl)	 
	{ 
	$sql = "SELECT * FROM $tbl ";	   
	$result = mysqli_query($this->conn,$sql) or die(mysqli_connect_errno()."Data cannot inserted"); 
